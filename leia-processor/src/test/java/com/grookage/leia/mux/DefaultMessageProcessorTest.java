...
class DefaultMessageProcessorTest {
	...
	void testHttpMessageProcessor() {
		...
		final var messageProcessor = new DefaultMessageProcessor("test", 10_000L, 1, resolver, executorFactory) {
			@Override
		...
		final var messageProcessor1 = new DefaultMessageProcessor("test", 10_000L, 1, resolver, executorFactory) {
			@Override
			protected boolean validBackends(Set<String> backends) {
				return true;
			}
		...
		leiaMessages.forEach(leiaMessage -> leiaMessage.setTags(Set.of("backend-backend1::backend2::backend3",
				"importance-mild::extreme")));
		final var messageProcessor2 = new DefaultMessageProcessor("test", 10_000L, 1, resolver, executorFactory) {
			@Override
			protected boolean validBackends(Set<String> backends) {
				return true;
			}
